            ...
            <div class="all-sort-list2" @click="toSearch">
              ...
              >
                <h3>
                  ...
                  <!-- 把所有a标签换成router-link会卡，因为组件标签太多了，导致内存当中组件对象很多，所以效率不高 -->
                  <!-- <router-link 
                  :to="{
                    name: 'search',
                    query: {
                  ...
                  <!-- 2.把声明式导航改为编程式导航，click是个事件，点击之后要调用函数，同样，每个
                  a标签都添加点击事件，那么内存中会定义很多函数，内存占用也很大，虽然效率高于声明式导航，但还不够完美 -->
                  <!-- <a 
                    href="javascript:;" 
                    @click=" 
                    $router.push({
                  ...
                  <!-- 事件委派：找公共的离自己最近的祖先元素，这个祖先元素只有一个 -->
                  <a href="javascript:;" 
...
// import _ from 'lodash' //引入整个lodash，打包后体积较大
...
export default {
  ...
  //组件在挂载完成的时候，就立马发送请求获取数据，存储到vuex里面，而不是直接在vue组件里面
  mounted() {
    //dispatch是分发和触发的意思，和emit单词意思一样
    //本质其实是在调用指定的action函数

    //如果在这发请求，home和search切换，内部都会重新创建typeNav组件，mounted就会重复执行
    //请求就会重复发，而三级分类列表数据是一样的，没必要发很多次
  ...
  //以后只要从vuex拿的是方法（mutations和actions当中的东西），都在methods当中去拿，一般用的少
  computed: {
    //函数内部可以是数组也可以是对象
    //是数组必须要满足条件：
    //1.数据直接就是总的state当中的数据，不能是模块里的
    //2.数组当中的名字必须和state当中的名字一致
    // ...mapState(['categoryList'])
    //不能这么写，因为catagoryList不是state.categoryList,而是state.home.categoryList
