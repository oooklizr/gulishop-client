...
export default {
  ...
  mounted() {
    ...
    //但是，这个页面当中结构中的swiper-slide，是根据请求回来的数据，动态创建生成的
    //所以，必须得保证请求数据回来之后，再去实例化，有了数据，slide的div才会动态创建ok
  ...
  //   watch: {
  //   bannerList: {
  //     immediate:true,//这里没什么用，就是为了和floor代码保持一致
  //     handler(newVal, oldVal) {
  //       //当数据一旦有变化，那么就去实例化swiper,但是发现不行
  //       //考虑是不是页面还是没形成呢？是的，也就说有了数据，上面页面才开始v-for形成结构
  //       //得等结构完全形成之后再去实例化
