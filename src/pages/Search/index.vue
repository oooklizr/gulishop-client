...
export default {
  ...
  data() {
    return {
      searchParams: {
        //这个对象称作叫初始化所有的搜索参数,只不过一上来所有的搜索条件，都是空的
        ...
        // 默认的搜索条件
        order: "2:desc", //排序规则，排序是后台排序的，搜索的时候得给后台一个默认的排序规则
        pageNo: 1, //搜素第几页的商品，分页也是后台做好的，也是得告诉后台我们要第几页数据
  ...
  mounted() {
    //点击跳转过来，是在这里发请求的
    //之前那样直接在mounted里面写,不好,一般是在mounted里面调用函数
    this.getSearchInfo();
  },
  methods: {
    getSearchInfo() {
      ...
      //但是点击三级分类或者点击搜索按钮跳过来search页面的时候，就已经是有搜索条件了
    ...
    removeCategoryName() {
      ...
      // this.getSearchInfo();
      //这里删除以后不会动我的原来路径，所以这样发请求不行，得让路径发生变化
